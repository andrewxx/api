...
### Запрос
`GET /resumes/mine/availability`

### Ответ

Успешный ответ приходит с кодом ответа `200 OK` и содержит в теле:
...
Имя  | Тип    | Описание
---- | ------ | ---
creation_available  | boolean | Флаг, который показывает доступно ли создание новых резюме для данного пользователя
max | number | Максимально возможное количество резюме
created  | number | Количество созданных резюме
remaining  | number | Количество резюме, которые ещё можно создать
